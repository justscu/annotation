...
ngx_event_find_timer(void)
{
    ...
    node = ngx_rbtree_min(root, sentinel); // find the least value. (the left.)

    ngx_mutex_unlock(ngx_event_timer_mutex);

    timer = (ngx_msec_int_t) (node->key - ngx_current_msec);//还有多长时间，定时时间到

    return (ngx_msec_t) (timer > 0 ? timer : 0);
}

// 查找红黑树中，哪些'定时器'的时间已经到了，并触发相关的回调函数
void
