	...
	if (c == NULL)
	{
		...
		if (r->cache->header_start + 256 >= u->conf->buffer_size)
		{
			ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "%V_buffer_size %uz is not enough for cache key, "
					"it should increased at least to %uz", &u->conf->module, u->conf->buffer_size, ngx_align(r->cache->header_start + 256, 1024));
...
static void ngx_http_upstream_upgrade(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	if (clcf->tcp_nodelay)
	{
		...
		if (u->peer.connection->tcp_nodelay == NGX_TCP_NODELAY_UNSET)
		{
			...
			{
				ngx_connection_error(u->peer.connection, ngx_socket_errno, "setsockopt(TCP_NODELAY) failed");
				ngx_http_upstream_finalize_request(r, u, 0);
				return;
			}
	...
	if (c->read->ready || r->header_in->pos != r->header_in->last)
	{
	...
	if (u->peer.connection)
	{
		...
		if (u->store)
		{
			...
			if (p->upstream_eof || p->upstream_done)
			{
				...
				if (u->headers_in.status_n == NGX_HTTP_OK && (u->headers_in.content_length_n == -1 || (u->headers_in.content_length_n == tf->offset)))
				{
		...
		if (u->cacheable)
		{
			...
			else if (p->upstream_eof)
			{
				...
				if (u->headers_in.content_length_n == -1 || u->headers_in.content_length_n == tf->offset - (off_t) r->cache->body_start)
				{
...
static ngx_int_t ngx_http_upstream_process_cache_control(ngx_http_request_t *r, ngx_table_elt_t *h, ngx_uint_t offset)
{
	...
	*ph = h;
...
#if (NGX_HTTP_CACHE)
	{
		...
		if (ngx_strlcasestrn(p, last, (u_char *) "no-cache", 8 - 1) != NULL || ngx_strlcasestrn(p, last, (u_char *) "no-store", 8 - 1) != NULL
...
static ngx_int_t ngx_http_upstream_copy_last_modified(ngx_http_request_t *r, ngx_table_elt_t *h, ngx_uint_t offset)
{
	...
	*ho = *h;
	...
	{
		r->headers_out.last_modified_time = ngx_http_parse_time(h->value.data, h->value.len);
	}
...
ngx_int_t ngx_http_upstream_cache_status(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data)
{
	ngx_uint_t n;
...
ngx_http_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
	...
	uscf = ngx_http_upstream_add(cf, &u,
			NGX_HTTP_UPSTREAM_CREATE | NGX_HTTP_UPSTREAM_WEIGHT | NGX_HTTP_UPSTREAM_MAX_FAILS | NGX_HTTP_UPSTREAM_FAIL_TIMEOUT | NGX_HTTP_UPSTREAM_DOWN | NGX_HTTP_UPSTREAM_BACKUP);
	if (uscf == NULL)
	{
...
ngx_http_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags)
{
	...
	for (i = 0; i < umcf->upstreams.nelts; i++)
	{
		...
		{
			ngx_log_error(NGX_LOG_WARN, cf->log, 0, "upstream \"%V\" may not have port %d in %s:%ui", &u->host, uscfp[i]->port, uscfp[i]->file_name, uscfp[i]->line);
			return NULL;
		}
...
ngx_int_t ngx_http_upstream_hide_headers_hash(ngx_conf_t *cf, ngx_http_upstream_conf_t *conf, ngx_http_upstream_conf_t *prev, ngx_str_t *default_hide_headers,
