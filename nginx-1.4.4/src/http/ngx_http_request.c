...
static ngx_int_t ngx_http_find_virtual_server(ngx_connection_t *c, ngx_http_virtual_names_t *virtual_names, ngx_str_t *host, ngx_http_request_t *r,
...
// 当监听到新的用户来的时候，调用的函数（在配置文件初始化时，被初始化）
void ngx_http_init_connection(ngx_connection_t *c)
{
	...
	rev = c->read;
	rev->handler = ngx_http_wait_request_handler; // handler
	c->write->handler = ngx_http_empty_handler; // handler
...
static void ngx_http_wait_request_handler(ngx_event_t *rev)
{
	...
	if (n == NGX_AGAIN)
	{
		...
		{
			ngx_log_error(NGX_LOG_INFO, c->log, NGX_ETIMEDOUT, "client timed out in deferred accept");
			ngx_http_close_connection(c);
			return;
		}
#endif
