...
// 根据 @sockaddr, @socklen，返回 ngx_listening_t结构
ngx_listening_t *
...
#if (NGX_HAVE_KEEPALIVE_TUNABLE) //维持心跳包的三个设置
...
#ifdef TCP_DEFER_ACCEPT // (延迟accept: 不是在客户端connect，而是在客户端真正发送数据过来的时候，才accept.)
...
ngx_int_t
ngx_connection_error(ngx_connection_t *c, ngx_err_t err, const char *text)
{
    ngx_uint_t  level;
