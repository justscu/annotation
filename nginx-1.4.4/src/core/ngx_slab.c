...
static ngx_uint_t  ngx_slab_exact_size; // 16
static ngx_uint_t  ngx_slab_exact_shift; // 4
...
ngx_slab_init(ngx_slab_pool_t *pool)
{
    ...
    if (ngx_slab_max_size == 0) {
        ...
        ngx_slab_exact_size = ngx_pagesize / (8 * sizeof(uintptr_t)); //16
        for (n = ngx_slab_exact_size; n >>= 1; ngx_slab_exact_shift++) {//4
            /* void */
        }
    }
    /**/
    ...
    n = ngx_pagesize_shift - pool->min_shift; //9
...
ngx_slab_alloc_locked(ngx_slab_pool_t *pool, size_t size)
{
    ...
    if (size >= ngx_slab_max_size) { // 分配一整页
