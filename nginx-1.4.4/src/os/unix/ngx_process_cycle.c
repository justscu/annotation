...
sig_atomic_t  ngx_reap; // 有子进程退出
sig_atomic_t  ngx_sigio;
sig_atomic_t  ngx_sigalrm;
sig_atomic_t  ngx_terminate;// SIGTERM/SIGILL
sig_atomic_t  ngx_quit; //退出，有清理动作 SIGQUIT
...
// 需要在配置文件中配置，才能够启动cache manager进程。
static void
...
ngx_cache_loader_process_handler(ngx_event_t *ev)
{
    ...
    for (i = 0; i < cycle->paths.nelts; i++) {
        ...
        if (path[i]->loader) {
            path[i]->loader(path[i]->data); //加载文件到缓存
            ngx_time_update();
        }
    }
    // 加载完成后，退出
