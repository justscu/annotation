/*
 * Copyright (C) 2010-2013 Wandenberg Peixoto <wandenberg@gmail.com>, Rog茅rio Carvalho Schneider <stockrt@gmail.com>
 *
 ...
 *
 * Created: Oct 26, 2010
 * Authors: Wandenberg Peixoto <wandenberg@gmail.com>, Rog茅rio Carvalho Schneider <stockrt@gmail.com>
 */
...
static ngx_int_t                                 ngx_http_push_stream_assing_subscription_to_channel_locked(ngx_slab_pool_t *shpool, ngx_str_t *channel_id, ngx_http_push_stream_subscription_t *subscription, ngx_http_push_stream_subscription_t *subscriptions_sentinel, ngx_log_t *log, ngx_uint_t* subers);
...
ngx_http_push_stream_subscriber_handler(ngx_http_request_t *r)
{
    ...
    if (!(r->method & NGX_HTTP_GET)) // 不是 GET ， 返回
    {
    ...
    polling = ((cf->location_type == NGX_HTTP_PUSH_STREAM_SUBSCRIBER_MODE_POLLING) ||
    		((push_mode != NULL) && (push_mode->len == NGX_HTTP_PUSH_STREAM_MODE_POLLING.len) &&
    				(ngx_strncasecmp(push_mode->data, NGX_HTTP_PUSH_STREAM_MODE_POLLING.data, NGX_HTTP_PUSH_STREAM_MODE_POLLING.len) == 0)));
    longpolling = ((cf->location_type == NGX_HTTP_PUSH_STREAM_SUBSCRIBER_MODE_LONGPOLLING) ||
    		((push_mode != NULL) && (push_mode->len == NGX_HTTP_PUSH_STREAM_MODE_LONGPOLLING.len) &&
    				(ngx_strncasecmp(push_mode->data, NGX_HTTP_PUSH_STREAM_MODE_LONGPOLLING.data, NGX_HTTP_PUSH_STREAM_MODE_LONGPOLLING.len) == 0)));

    if (polling || longpolling)
    {
        ngx_int_t result = ngx_http_push_stream_subscriber_polling_handler(r, channels_ids, if_modified_since, tag, \
        		last_event_id, longpolling, ctx->temp_pool);
...
static ngx_int_t
ngx_http_push_stream_subscriber_polling_handler(ngx_http_request_t *r, ngx_http_push_stream_requested_channel_t *channels_ids, \
		time_t if_modified_since, ngx_int_t tag, ngx_str_t *last_event_id, ngx_flag_t longpolling, ngx_pool_t *temp_pool)
{
    ngx_http_push_stream_main_conf_t               *mcf = ngx_http_get_module_main_conf(r, ngx_http_push_stream_module);
    ngx_http_push_stream_loc_conf_t                *cf = ngx_http_get_module_loc_conf(r, ngx_http_push_stream_module);
    ngx_slab_pool_t                                *shpool = mcf->shpool;
    ...
    while ((cur = (ngx_http_push_stream_requested_channel_t *) ngx_queue_next(&cur->queue)) != channels_ids)
    {
        ...
        if (ngx_http_push_stream_has_old_messages_to_send(channel, cur->backtrack_messages, if_modified_since, tag, \
        		greater_message_time, greater_message_tag, last_event_id))
        {
    ...
    if (longpolling && !has_message_to_send) {
        ...
        while ((cur = (ngx_http_push_stream_requested_channel_t *) ngx_queue_next(&cur->queue)) != channels_ids)
        {
            ...
            ngx_uint_t subers = 0;
            ngx_http_push_stream_assing_subscription_to_channel_locked(shpool, cur->id, subscription, \
            		&worker_subscriber->subscriptions_sentinel, r->connection->log, &subers);

        	df_sensus_subscribers((const char*)cur->id->data, cur->id->len, channel->subscribers, TYPE_OPER_Subscribe);
        }
...
static ngx_int_t
ngx_http_push_stream_subscriber_assign_channel(ngx_http_push_stream_main_conf_t *mcf, ngx_http_push_stream_loc_conf_t *cf, \
		ngx_http_request_t *r, ngx_http_push_stream_requested_channel_t *requested_channel, time_t if_modified_since, \
		ngx_int_t tag, ngx_str_t *last_event_id, ngx_http_push_stream_subscriber_t *subscriber, ngx_pool_t *temp_pool)
{
    ngx_http_push_stream_channel_t             *channel;
    ngx_http_push_stream_subscription_t        *subscription;
    ngx_int_t                                   result;
    ...
    if ((channel = ngx_http_push_stream_find_channel(requested_channel->id, r->connection->log, mcf)) == NULL) {
        // channel not found
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "push stream module: unable to allocate shared memory for channel %s", \
        		requested_channel->id->data);
        return NGX_ERROR;
    }
    ...
    ngx_uint_t subers = 0;
    ngx_shmtx_lock(&shpool->mutex);
    result = ngx_http_push_stream_assing_subscription_to_channel_locked(shpool, requested_channel->id, subscription, \
    		&subscriber->subscriptions_sentinel, r->connection->log, &subers);
    ngx_shmtx_unlock(&shpool->mutex);

    if(result == NGX_OK)
    {
		df_sensus_subscribers((const char*)requested_channel->id->data, requested_channel->id->len, channel->subscribers, TYPE_OPER_Subscribe);
    }
...
ngx_http_push_stream_validate_channels(ngx_http_request_t *r, ngx_http_push_stream_requested_channel_t *channels_ids, ngx_int_t *status_code, ngx_str_t **explain_error_message)
{
    ...
    while ((cur = (ngx_http_push_stream_requested_channel_t *) ngx_queue_next(&cur->queue)) != channels_ids) {
        // could not be ALL channel or contain wildcard, 通道的名字不能为 'ALL' 和 '*'
        ...
        // could not have a large size，检查是否超过了通道名字最大长度
        ...
        // count subscribed normal and wildcard channels，通配符的前缀
...
// @subers: 传出参数，订阅人数
static ngx_int_t
ngx_http_push_stream_assing_subscription_to_channel_locked(ngx_slab_pool_t *shpool, ngx_str_t *channel_id, \
		ngx_http_push_stream_subscription_t *subscription, ngx_http_push_stream_subscription_t *subscriptions_sentinel, \
		ngx_log_t *log, /*out*/ngx_uint_t* subers)
{
...
ngx_http_push_stream_get_padding_by_user_agent(ngx_http_request_t *r)
{
    ...
    if (cf->user_agent != NULL) {
        ...
        vv_user_agent = r->headers_in.user_agent->value; // curl/7.35.0，代理的版本
    }
