...
typedef struct {
    ...
    time_t                          message_ttl;
    ngx_uint_t                      max_subscribers_per_channel; // 每个通道的最大订阅用户数
    ngx_uint_t                      max_messages_stored_per_channel;//每个通道保存的最大消息数
    ngx_uint_t                      max_channel_id_length; // 通道名字的最大长度
    ...
    ngx_http_push_stream_shm_data_t*shm_data; //共享内存
...
typedef struct {
    ...
    ngx_str_t                           id;
    ngx_uint_t                          last_message_id;// 最后一次发布的消息的ID
    time_t                              last_message_time;
    ngx_int_t                           last_message_tag;
    ngx_uint_t                          stored_messages;
    ngx_uint_t                          subscribers; //通道订阅数
    ngx_queue_t                         workers_with_subscribers; //本队列中放的是订阅者
    ngx_queue_t                         message_queue;
    time_t                              expires; //该通道超时时间
    ngx_flag_t                          deleted; //该通道是否被删除了
    ngx_flag_t                          wildcard;
...
struct ngx_http_push_stream_subscriber_s {
    ...
    ngx_http_push_stream_subscription_t         subscriptions_sentinel; //订阅_哨兵
...
typedef struct {
    ...
    ngx_http_push_stream_subscriber_t  *subscriber; // 存放订阅信息
    ngx_flag_t                          longpolling;
...
typedef struct {
    ...
    ngx_uint_t                          subscribers; // # of subscribers in the worker ， 本worker进程订阅数
    time_t                              startup;
    pid_t                               pid;
...
struct ngx_http_push_stream_shm_data_s {
    ...
    time_t                                  last_message_time;
    ngx_int_t                               last_message_tag;
    ngx_queue_t                             shm_data_queue; //共享数据队列
    ngx_http_push_stream_main_conf_t       *mcf;
    ngx_shm_zone_t                         *shm_zone;
    ngx_slab_pool_t                        *shpool;
};
