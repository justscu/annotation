...
class Harness
{
	public:
				...
				const KVMap& data)
		{
			...
			iter->SeekToFirst();
			for (KVMap::const_iterator model_iter = data.begin(); model_iter
					!= data.end(); ++model_iter)
			{
				...
				const KVMap& data)
		{
			...
			ASSERT_TRUE(!iter->Valid());
			iter->SeekToLast();
			for (KVMap::const_reverse_iterator model_iter = data.rbegin(); model_iter
					!= data.rend(); ++model_iter)
			{
				ASSERT_EQ(ToString(data, model_iter), ToString(iter));
				iter->Prev();
			}
...
TEST(Harness, Randomized)
{
	for (int i = 0; i < kNumTestArgs; i++)
	{
		...
		for (int num_entries = 0; num_entries < 2000; num_entries
				+= (num_entries < 50 ? 1 : 200))
		{
			...
			{
				std::string v;
				Add(test::RandomKey(&rnd, rnd.Skewed(4)), test::RandomString(
						&rnd, rnd.Skewed(5), &v).ToString());
			}
			Test(&rnd);
		}
	}
...
TEST(Harness, RandomizedLongDB)
{
	...
	{
		std::string v;
		Add(test::RandomKey(&rnd, rnd.Skewed(4)), test::RandomString(&rnd,
				rnd.Skewed(5), &v).ToString());
	}
	Test(&rnd);
