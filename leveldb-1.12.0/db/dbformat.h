...
static const int kL0_CompactionTrigger = 4; //触发压缩
...
// SequenceNumber 是leveldb很重要的东西，每次对数据库进行更新操作，
// 都会生成一个新的SequenceNumber,64bits，其中高8位为0，可以跟key的类型(8bits)进行合并成64bits。
...
//解析InternalKey, 都变成可见字符，若不可见，用 "\x0a" 这种16进制方式打印出来
...
extern void
AppendInternalKey(std::string* result, const ParsedInternalKey& key);
...
//
// InternalKey包括 key + sequence + type.
//
...
// 从 @internal_key中解析出来，并将结果放到 @result中
// 获取 key, sequence, type
