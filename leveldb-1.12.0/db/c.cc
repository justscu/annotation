...
{
		void* state_;
		void (*destructor_)(void*); // 析构，销毁该结构
		int (*compare_)(void*, const char* a, size_t alen, const char* b,
				size_t blen);
		const char* (*name_)(void*); // 返回name
...
struct leveldb_filterpolicy_t: public FilterPolicy
{
		...
		char
		* (*create_)(void*, const char* const * key_array,
		...
		// @n，key的个数
		// @keys，存放key的数组
		// func: 根据keys，生成 filter，并将生成的 filter放入到 @dst 中。
		// @dst，存放的 filter.
		virtual void CreateFilter(const Slice* keys, int n, std::string* dst) const
		{
			...
			for (int i = 0; i < n; i++)
			{
				key_pointers[i] = keys[i].data(); // 存放所有的key
				key_sizes[i] = keys[i].size(); // 存放key的大小
			}
			size_t len;
			char* filter = (*create_)(state_, &key_pointers[0], &key_sizes[0],
					n, &len);
...
static bool SaveError(char** errptr, const Status& s)
{
	...
	else
	{
		...
		free(*errptr);
		*errptr = strdup(s.ToString().c_str()); // 需要自己释放内存
	}
	return true;
}
...
void leveldb_put(leveldb_t* db, const leveldb_writeoptions_t* options,
		...
		char** errptr)
{
	SaveError(errptr, db->rep->Put(options->rep, Slice(key, keylen), Slice(val,
			vallen)));
}
...
void leveldb_writebatch_iterate(leveldb_writebatch_t* b, void* state,
		void(*put)(void*, const char* k, size_t klen, const char* v,
				size_t vlen), void(*deleted)(void*, const char* k, size_t klen))
{
	class H: public WriteBatch::Handler
	{
		public:
			...
			{
				(*put_)(state_, key.data(), key.size(), value.data(),
						value.size());
			}
...
leveldb_comparator_t* leveldb_comparator_create(void* state, void(*destructor)(
		void*), int(*compare)(void*, const char* a, size_t alen, const char* b,
...
//过滤策略，生成一个bloom过滤器，每key有10个bits
leveldb_filterpolicy_t* leveldb_filterpolicy_create_bloom(int bits_per_key)
{
	...
	wrapper->rep_ = NewBloomFilterPolicy(bits_per_key); //指向基类指针
	wrapper->state_ = NULL;
