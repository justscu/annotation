...
// 判断 @user_key 是否大于 @f 中的最大key
static bool AfterFile(const Comparator* ucmp, const Slice* user_key,
		const FileMetaData* f)
{
	// NULL user_key occurs before all keys and is therefore never after *f
	return (user_key != NULL && ucmp->Compare(*user_key, f->largest.user_key())
			> 0);
}

// 判断 @user_key 是否小于 @f 中的最小key
...
// 判断 [@smallest_user_key, @largest_user_key]是否与 @files中的key重叠
// @disjoint_sorted_files == true，表明 @files中的key是排序的
bool SomeFileOverlapsRange(const InternalKeyComparator& icmp,
		bool disjoint_sorted_files, const std::vector<FileMetaData*>& files,
		const Slice* smallest_user_key, const Slice* largest_user_key)
{
	const Comparator* ucmp = icmp.user_comparator();
	// @files 没有排序，要检查所有的file [level0]
	if ( !disjoint_sorted_files )
	{
		...
		for (size_t i = 0; i < files.size(); i++)
		{
			...
			if ( AfterFile(ucmp, smallest_user_key, f) || BeforeFile(ucmp,
					largest_user_key, f) )
			{
				// No overlap
			}
...
// Concatenating(把 。。。联系起来)
Iterator* Version::NewConcatenatingIterator(const ReadOptions& options,
		int level) const
{
	return NewTwoLevelIterator(new LevelFileNumIterator(vset_->icmp_,
			&files_[level]), &GetFileIterator, vset_->table_cache_, options);
}
		...
		std::vector<Iterator*>* iters)
{
	...
	{
		iters->push_back(vset_->table_cache_->NewIterator(options,
...
{
	Slice ikey = k.internal_key(); // key+sequence+type
	Slice user_key = k.user_key(); // key
	const Comparator* ucmp = vset_->icmp_.user_comparator();
	Status s;

	stats->seek_file = NULL; //查找的文件指针
	stats->seek_file_level = -1; //查找的文件级别
	...
	std::vector<FileMetaData*> tmp; // 满足条件的文件
	FileMetaData* tmp2;
	for (int level = 0; level < config::kNumLevels; level++)
	{
		size_t num_files = files_[level].size(); //每一级，有多少文件
		if ( num_files == 0 )
			continue;
		...
		if ( level == 0 )
		{
			...
			for (uint32_t i = 0; i < num_files; i++)
			{
				...
				{
					tmp.push_back(f); // user_key在最大key & 最小key之间
				}
			}
			if ( tmp.empty() )
				continue;
...
// 更新文件的状态，当 allowed_seeks<=0时，触发压缩.
// @return，= true，需要压缩
...
//  判断 [@smallest_user_key, @largest_user_key]是否跟现有的文件中的key有重叠
...
// 把与[@begin, @end]重叠的部分，都放到@inputs中；
	...
	for (size_t i = 0; i < files_[level].size();)
	{
		...
		else
		{
			...
			if ( level == 0 )
			{
				...
				if ( begin != NULL && user_cmp->Compare(file_start, user_begin)
						< 0 )
				{
...
class VersionSet::Builder
{
	...
	public:
		...
		void Apply(VersionEdit* edit)
		{
			...
			for (VersionEdit::DeletedFileSet::const_iterator iter = del.begin(); iter
					!= del.end(); ++iter)
			{
		...
		void SaveTo(Version* v)
		{
			...
			for (int level = 0; level < config::kNumLevels; level++)
			{
				...
				for (FileSet::const_iterator added_iter = added->begin(); added_iter
						!= added->end(); ++added_iter)
				{
					// Add all smaller files listed in base_
					for (std::vector<FileMetaData*>::const_iterator bpos =
							std::upper_bound(base_iter, base_end, *added_iter,
									cmp); base_iter != bpos; ++base_iter)
					{
...
VersionSet::VersionSet(const std::string& dbname, const Options* options,
	...
	env_(options->env), dbname_(dbname), options_(options), table_cache_(
			...
			dummy_versions_(this), current_(NULL)
{
...
// @dummy_versions_指向头节点；
// 运行完毕后，@current_ = @v;
// 同时将@v添加到@dummy_versions_的前面；即循环链表的最后一个
...
Status VersionSet::Recover()
{
...

	{
		...
		reporter.status = &s;
		log::Reader
				reader(file, &reporter, true/*checksum*/, 0/*initial_offset*/);
		Slice record;
		std::string scratch;
		while (reader.ReadRecord(&record, &scratch) && s.ok())
		{
			...
			if ( s.ok() )
			{
				if ( edit.has_comparator_ && edit.comparator_
		...
		for (size_t i = 0; i < files.size(); i++)
		{
			...
			else // 找到对应的文件了
			{
				...
				Iterator* iter = table_cache_->NewIterator(ReadOptions(),
						files[i]->number, files[i]->file_size, &tableptr);
				if ( tableptr != NULL )
				{
...
// 将所有 version中的文件的number，加入到 @live中。
void VersionSet::AddLiveFiles(std::set<uint64_t>* live)
{
	for (Version* v = dummy_versions_.next_; v != &dummy_versions_; v
			= v->next_)
	{
		for (int level = 0; level < config::kNumLevels; level++)
		{
...
// 将@inputs中最小的放在 @smallest，最大放在 @largest.
			...
			else
			{
				...
				list[num++] = NewTwoLevelIterator(
						new Version::LevelFileNumIterator(icmp_,
								&c->inputs_[which]), &GetFileIterator,
...
Compaction* VersionSet::PickCompaction()
{
	...
	if ( size_compaction )
	{
		...
		for (size_t i = 0; i < current_->files_[level].size(); i++)
		{
			...
			if ( compact_pointer_[level].empty() || icmp_.Compare(
					f->largest.Encode(), compact_pointer_[level]) > 0 )
			{
...
void VersionSet::SetupOtherInputs(Compaction* c)
{
	...
	if ( !c->inputs_[1].empty() )
	{
		...
		if ( expanded0.size() > c->inputs_[0].size() && inputs1_size
				+ expanded0_size < kExpandedCompactionByteSizeLimit )
		{
			...
			if ( expanded1.size() == c->inputs_[1].size() )
			{
				Log(
						...
						int(c->inputs_[1].size()), long(inputs0_size),
						long(inputs1_size), int(expanded0.size()),
...
bool Compaction::ShouldStopBefore(const Slice& internal_key)
{
	...
	while (grandparent_index_ < grandparents_.size() && icmp->Compare(
			internal_key, grandparents_[grandparent_index_]->largest.Encode())
			> 0)
	{
		if ( seen_key_ )
		{
