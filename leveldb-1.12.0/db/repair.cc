...
class Repairer
{
	public:
		Repairer(const std::string& dbname, const Options& options) :
			dbname_(dbname), env_(options.env), icmp_(options.comparator),
					ipolicy_(options.filter_policy), options_(SanitizeOptions(
							dbname, &icmp_, &ipolicy_, options)),
	...
	private:
		...
		Status ConvertLogToTable(uint64_t log)
		{
			...
			// numbers).
			log::Reader
					reader(lfile, &reporter, false/*do not checksum*/, 0/*initial_offset*/);
		...
		void ExtractMetaData()
		{
			...
			for (size_t i = 0; i < table_numbers_.size(); i++)
			{
				...
				if ( !status.ok() )
				{
					std::string fname = TableFileName(dbname_,
							table_numbers_[i]);
